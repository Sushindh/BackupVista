...
export const demoFetchPanels = async (filters = {}) => {
  await delay();
  console.log("[DEMO] Fetching panels with filters:", filters);
  let data = [...DUMMY_PANELS];

  // More flexible filtering - check both 'program' and 'department' fields
  if (filters.school) {
    data = data.filter((p) => p.school === filters.school);
  }
  if (filters.program) {
    data = data.filter(
      (p) => p.program === filters.program || p.department === filters.program
    );
  }
  if (filters.department) {
    data = data.filter(
      (p) =>
        p.program === filters.department || p.department === filters.department
    );
  }
  if (filters.academicYear) {
    data = data.filter((p) => p.academicYear === filters.academicYear);
  ...
  console.log("[DEMO] Returning panels:", data.length, "panels");
...
export const demoFetchPanelSummary = async (filters = {}) => {
  ...
  return {
    ...
    assignedProjects: panels.reduce(
      (sum, p) => sum + p.assignedProjectsCount,
      0
    ),
  };
};
...
export const demoFetchStudentMarks = async (studentId, reviewType) => {
  ...
  const data = DUMMY_MARKS.filter(
    (m) =>
      m.student === studentId && (!reviewType || m.reviewType === reviewType)
  );
  return { success: true, data };
};
    ...
    try {
      ...
      if (user.role === "admin") {
        return {
          ...
          data: {
            isPrimary: true,
            permissions: { canEdit: { enabled: true } },
          },
        };
      }
...
export const demoFetchReviewProgress = async (filters = {}) => {
  ...
  const reviewProgress = {
    review1: {
      ...
      completed: DUMMY_MARKS.filter(
        (m) => m.reviewType === "review1" && m.isSubmitted
      ).length,
      pending:
        projects.length * 2 -
        DUMMY_MARKS.filter((m) => m.reviewType === "review1" && m.isSubmitted)
          .length,
    },
    review2: {
      total: projects.length * 2,
      completed: DUMMY_MARKS.filter(
        (m) => m.reviewType === "review2" && m.isSubmitted
      ).length,
      pending:
        projects.length * 2 -
        DUMMY_MARKS.filter((m) => m.reviewType === "review2" && m.isSubmitted)
          .length,
    },
    review3: {
      total: projects.length * 2,
      completed: DUMMY_MARKS.filter(
        (m) => m.reviewType === "review3" && m.isSubmitted
      ).length,
      pending:
        projects.length * 2 -
        DUMMY_MARKS.filter((m) => m.reviewType === "review3" && m.isSubmitted)
          .length,
    },
  };
...
export const demoFetchMarksReport = async (filters = {}) => {
  ...
  const reports = DUMMY_STUDENTS.map((student) => {
    ...
    const project = DUMMY_PROJECTS.find((p) =>
      p.students.includes(student._id)
    );
...
// =============================================
// MODIFICATION / REASSIGNMENT APIs
// =============================================

/**
 * Get guide projects grouped by faculty
 */
export const demoGetGuideProjects = async (
  academicYear,
  school,
  department
) => {
  ...
  // Group projects by guide faculty
  const guideProjectsMap = {};
  ...
  DUMMY_PROJECTS.filter(
    ...
    if (!guideProjectsMap[guideId]) {
      const faculty = DUMMY_FACULTY.find((f) => f._id === guideId);
    ...
    guideProjectsMap[guideId].guidedProjects.push({
      _id: project._id,
      name: project.name,
  ...
  return { success: true, data: Object.values(guideProjectsMap) };
};

/**
 * Get panel projects grouped by panel
 */
export const demoGetPanelProjects = async (
  academicYear,
  school,
  department
) => {
  ...
  // Group projects by panel
  const panelProjectsMap = {};
  ...
  DUMMY_PROJECTS.filter(
    ...
    panelProjectsMap[panelId].projects.push({
      _id: project._id,
      name: project.name,
      students: project.studentDetails || [],
      status: project.status,
  ...
  return { success: true, data: Object.values(panelProjectsMap) };
};

/**
 * Get all panels for academic context
 */
export const demoGetPanels = async (academicYear, school, department) => {
  await delay();

  const panels = DUMMY_PANELS.filter(
    (p) =>
      p.academicYear === academicYear &&
      p.school === school &&
      p.program === department
